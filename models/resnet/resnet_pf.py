...
__all__ = ['pf_resnet18', 'pf_resnet34', 'pf_resnet50', 'pf_resnet101', 'pf_resnet152']
...
class ParamFree(nn.Module):
    ...
    def forward(self, x):
        ...
        similarity = self.sig(self.scale*similarity.unsqueeze(dim=1))
...
def pf_resnet18(pretrained=False, **kwargs):
...
def pf_resnet34(pretrained=False, **kwargs):
...
def pf_resnet50(pretrained=False, **kwargs):
...
def pf_resnet101(pretrained=False, **kwargs):
...
def pf_resnet152(pretrained=False, **kwargs):
...
def demo():
    ...
    for i in range(1):
        net = pf_resnet50(num_classes=1000)
...
def demo2():
    st = time.perf_counter()
    for i in range(100):
        net = pf_resnet50(num_classes=1000).cuda()
