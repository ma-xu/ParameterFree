...
__all__ = ['pf7_resnet18', 'pf7_resnet34', 'pf7_resnet50', 'pf7_resnet101', 'pf7_resnet152']
...
class ParamFree(nn.Module):
    ...
    def forward(self, x):
        ...
        min,_ = similarity.min(dim=2,keepdim=True)
        max,_ = similarity.max(dim=2,keepdim=True)
...
def pf7_resnet18(pretrained=False, **kwargs):
...
def pf7_resnet34(pretrained=False, **kwargs):
...
def pf7_resnet50(pretrained=False, **kwargs):
...
def pf7_resnet101(pretrained=False, **kwargs):
...
def pf7_resnet152(pretrained=False, **kwargs):
...
def demo():
    ...
    for i in range(1):
        net = pf7_resnet50(num_classes=1000)
...
def demo2():
    st = time.perf_counter()
    for i in range(100):
        net = pf7_resnet50(num_classes=1000).cuda()
